...
describe("departments", () => {
	...
	it("创建测试数据", async () => {
		const [sales, departmentA, departmentB, departmentC] =
	...
	it("验证部门关系", async () => {
		...
		const domesticSales = allDepartments.find(
			(dept) => dept.name === "Domestic Sales",
		)
		const internationalSales = allDepartments.find(
			(dept) => dept.name === "International Sales",
		)
		const onlineSales = allDepartments.find(
			(dept) => dept.name === "Online Sales",
		)
		...
		const salesToDomestic = relationships.find(
			(rel) =>
				rel.departmentId === sales!.id &&
				rel.relatedDepartmentId === domesticSales!.id,
		)
		const salesToInternational = relationships.find(
			(rel) =>
				rel.departmentId === sales!.id &&
				rel.relatedDepartmentId === internationalSales!.id,
		)
		const domesticToOnline = relationships.find(
			(rel) =>
				rel.departmentId === domesticSales!.id &&
				rel.relatedDepartmentId === onlineSales!.id,
		)
